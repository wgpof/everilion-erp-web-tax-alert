//JFT 07/05/2012 New Function for check file size.
// checkFileSize:
//  if IE, check file size with ActiveX
//  else check with JavaScript
// Parameters:
//  inputField: path and name of the input in the form.
//  maxSize: maximum size for the file.
function checkFileSize(inputField, maxSize) {
    if (navigator.appName == "Microsoft Internet Explorer") {
        var fso = new ActiveXObject("Scripting.FileSystemObject");
        if (inputField.value != "") {
            if (fso.GetFile(inputField.value).Size > maxSize) {
                return false;
            }
        }
    }
